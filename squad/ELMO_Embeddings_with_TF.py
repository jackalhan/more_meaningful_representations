...
def get_elmo_embeddings(tokenized_questions, tokenized_paragraphs, token_embeddings_guideline_file,
    ...
    if not os.path.exists(token_embeddings_guideline_file):
            ...
            corpus_as_tokens = tokenized_questions + tokenized_paragraphs
            corpus_as_tokens = corpus_as_tokens if resource['is_demo_slice'] is None else resource['is_demo_slice']
            ...
            if not is_google_elmo:
                ...
                ******************************************************************************************************************
                ...
                if os.path.exists(token_embeddings_file.replace('@@', str('old_api_@@')).replace('@@', total_parition_size_for_old_api)):
                    print('ELMO embeddings are generated by using old API')
                    for partition_index in range(1, total_parition_size_for_old_api + 1):
                        with h5py.File(token_embeddings_file.replace('@@', str('old_api_@@')).replace('@@', partition_index), 'r') as fin:
                ...
                print('OLD and NEW Embeddings are being written')
