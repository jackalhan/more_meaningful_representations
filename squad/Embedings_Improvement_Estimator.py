...
def predictor(estimator, base_data_path, params):
    predictions = estimator.predict(lambda: predict_input_fn(base_data_path, params))
...
def execute_non_conv_pipeline(params, base_data_path, config, tf):
    ...
    if not params.executor["is_prediction"]:
        ...
        _train_input_fn = lambda: train_input_fn(base_data_path,
                                                 params)
        _test_recall_input_fn = lambda: test_recall_input_fn(base_data_path,
                                               params)
        ...
        if not params.executor["is_debug_mode"]:
            ...
            # -------------------------------------
                ...
                # Then Test
                # -------------------------------------
            estimator.evaluate(_test_recall_input_fn)

                # -------------------------------------
                # Train and Test : Test
                # -------------------------------------
            test_spec = tf.estimator.EvalSpec(_test_recall_input_fn) #steps=params.model['num_epochs']
            ...
            for ep in range(1,params.model["num_epochs"] + 1):
                ...
                result_as_dict_list = estimator.predict(_test_recall_input_fn)
...
def execute_conv_pipeline(params, base_data_path, config, tf):
    ...
    if not params.executor["is_prediction"]:
        ...
        _train_input_fn = lambda: train_input_fn(base_data_path, params)
        _test_recall_input_fn = lambda: test_recall_input_fn(base_data_path, params)
        _train_recall_input_fn = lambda: train_recall_input_fn(base_data_path, params)
        if not params.executor["is_debug_mode"]:
