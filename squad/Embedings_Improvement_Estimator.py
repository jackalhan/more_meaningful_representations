...
import numpy as np
from helper.utils import Params, define_pre_executions,dump_embeddings
...
if __name__ == '__main__':
    ...
    config = tf.estimator.RunConfig(tf_random_seed=params.model["seed"],
                                    model_dir=params.executor["model_dir"],
                                    save_summary_steps=params.model["save_summary_steps"])
    ...
    _test_input_fn = lambda: test_input_fn(base_data_path,
                                           params)
    # -------------------------------------
    # Baseline Train for Initial model
    # -------------------------------------
    ...
    test_spec = tf.estimator.EvalSpec(_test_input_fn)
    #, steps=params.model['num_epochs']
    # estimator.evaluate(_test_input_fn)
