...
def dump_embeddings(embeddings, outfile_to_dump):
    _e = outfile_to_dump.rpartition(os.path.sep)
    path_e = _e[0]
    new_embedding_embed_file = os.path.join(path_e, 'new_train' + _e[2].replace('train', ''))

    with h5py.File(new_embedding_embed_file, 'w') as fout:
